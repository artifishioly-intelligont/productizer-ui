...
class Deepzoom
{
     ...
     * @return array|string
     */
    public function makeTiles($image, $map, $file = NULL, $folder = NULL)
    {
        ...
        foreach(range($numLevels - 1, $lowestLevel) as $level) {
            ...
            $this->createLevelTiles($dimension['width'], $dimension['height'], $level, $level_folder, $img, $map);
        }
        ...
        $map->levels = $numLevels;
        $map->rows = $this->getNumRows($height);
        $map->columns = $this->getNumColumns($width);
        $map->save();
     ...
     * @return int
     */
    public function getNumColumns($width)
    {
        $columns = (int)ceil(floatval($width) / $this->tileSize);
        return $columns;
    }
     ...
     * @return int
     */
    public function getNumRows($height)
    {
        $rows = (int)ceil(floatval($height) / $this->tileSize);
        return $rows;
    }
     ...
     * @param $img
     */
    public function createLevelTiles($width, $height, $level, $folder, $img, $map)
    {
